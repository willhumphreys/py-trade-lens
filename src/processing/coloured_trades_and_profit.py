...
def plot_data(df, trader_id, output_path):
...
def process_and_plot_file(file_name, input_dir, output_dir):
    start_time = time.time()
    df, trader_id = process_file(file_name, input_dir)
    plot_data(df, trader_id, output_dir)
    # Cleanup
    del df
    gc.collect()
...
def process_and_plot_files(input_dir, output_dir):
    create_output_directory(output_dir)
    files = [f for f in os.listdir(input_dir) if f.endswith('.csv')]
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        for file_name in files:
            futures.append(executor.submit(process_and_plot_file, file_name, input_dir, output_dir))
...
if __name__ == "__main__":
    ...
    for scenario_dir in os.listdir(base_scenario_dir):
        ...
        if os.path.isdir(scenario_path):
            ...
            process_and_plot_files(input_dir, graph_output_dir)
            gc.collect()
